...
public class Input extends BaseTest {
	...
	public void inputPhoneNumberForgot(String keyword) {
		wait = new WebDriverWait(android, Duration.ofSeconds(60));
		WebElement phoneNumberElement = android.findElement(
				By.xpath("//android.widget.TextView[@text='Phone Number']/following::android.widget.EditText[1]"));
	...
	public void inputEmailAgain(String keyword) {
		// Assuming the field note element is represented by txt_fld_note in your
		// RegisterPage class
	...
	public void inputPasswordDeleteAccount(String keyword) {
		try {
			settingPage = new SettingPage(android);
			wait = new WebDriverWait(android, Duration.ofSeconds(60));
			WebElement enter_password = settingPage.field_delete_account_enter_password;
			enter_password.sendKeys(keyword);
			System.out.println("Success Input Password");
		} catch (Exception e) {
			System.out.println("Failed Input Password");
			e.printStackTrace();
		}
	}

	public void clearInputPasswordDeleteAccount() {
		settingPage = new SettingPage(android);
		wait = new WebDriverWait(android, Duration.ofSeconds(90));
		WebElement fld_enter_password = settingPage.field_delete_account_enter_password;
		fld_enter_password.clear();
	}
}